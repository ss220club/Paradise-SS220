...
import { Box, Button, Dropdown, Input, LabeledList, Section, Stack, TextArea } from 'tgui-core/components';
      ...
      <Stack fill vertical>
        <Stack.Item>
          <Section
            ...
            buttons={<Button icon="arrow-circle-left" content="Back to Nexus" onClick={() => setShowAnnounce(false)} />}
          >
            <Stack fill vertical>
              <Stack.Item>
                <Input
                  fluid
                  placeholder="From (Большой белый заголовок)..."
                  value={announceData.subtitle}
                  onChange={(value) => setAnnounceData({ ...announceData, subtitle: value })}
                />
              </Stack.Item>
              <Stack.Item>
                <Input
                  fluid
                  placeholder="Topic (Меньший белый заголовок)..."
                  value={announceData.title}
                  onChange={(value) => setAnnounceData({ ...announceData, title: value })}
                />
              </Stack.Item>
              <Stack.Item>
                <TextArea
                  fluid
                  height="120px"
                  placeholder="Enter Announcement text here (Red small font)..."
                  value={announceData.text}
                  onChange={(value) => setAnnounceData({ ...announceData, text: value })}
                />
              </Stack.Item>
              <Stack.Item>
                <Stack align="center">
                  <Stack.Item grow>
                    <Dropdown
                      options={possible_cc_sounds || ['Beep']}
                      selected={announceData.beepsound}
                      onSelected={(val) => setAnnounceData({ ...announceData, beepsound: val })}
                      disabled={announceData.classified}
                    />
                  </Stack.Item>
                  <Stack.Item>
                    <Button
                      icon="volume-up"
                      disabled={announceData.classified}
                      tooltip="Test sound locally"
                      onClick={() => act('test_sound', { sound: announceData.beepsound })}
                    />
                  </Stack.Item>
                  <Stack.Item>
                    <Button.Checkbox
                      fluid
                      checked={announceData.classified}
                      tooltip={
                        announceData.classified ? 'Sent to station communications consoles' : 'Publically announced'
                      }
                      onClick={() => setAnnounceData({ ...announceData, classified: announceData.classified ? 0 : 1 })}
                    >
                      Classified
                    </Button.Checkbox>
                  </Stack.Item>
              ...
              <Stack.Item>
                <Button
                  ...
                  textAlign="center"
                  color="good"
                  disabled={announceData.text.length < 6}
                  onClick={() => {
                    act('make_cc_announcement', {
                      subtitle: announceData.subtitle,
                      title: announceData.title,
                      text: announceData.text,
                      beepsound: announceData.beepsound,
                      classified: announceData.classified,
                    });
                    setAnnounceData({
                      subtitle: `Отчёт от ${ai_name || 'Nexus'}`,
                      title: 'Nexus Broadcast',
