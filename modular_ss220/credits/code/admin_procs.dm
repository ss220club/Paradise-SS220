...
USER_VERB(toggle_credits, R_ADMIN, "Toggle Credits", "Просмотр титров по окончании раунда", VERB_CATEGORY_SERVER)
	...
	if(GLOB.credits_forced)
		...
		message_admins("[key_name_admin(client)] устанавливает принудительные титры.", 1)
	else
		to_chat(world, "<B>Игроки будут смотреть титры в зависимости от своих настроек.</B>")
		message_admins("[key_name_admin(client)] устанавливает титры по умолчанию.", 1)
	log_admin("[key_name(client)] toggled credits.")
