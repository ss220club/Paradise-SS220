...
/mob/living/simple_animal/hostile/poison/terror_spider/prince
	...
	death_sound = 'modular_ss220/events/sound/creatures/terrorspiders/prince_dead.ogg'
	...
	attack_sound = 'modular_ss220/events/sound/creatures/terrorspiders/bite2.ogg'
	ventcrawler = null
	ai_ventcrawls = FALSE
	...
	web_type = null
	special_abillity = list(/datum/spell/aoe/terror_slam)
...
/mob/living/simple_animal/hostile/poison/terror_spider/prince/spider_specialattack(mob/living/carbon/human/L)
	if(prob(delimb_chance))
		if(L.stat != DEAD) //no dismemberment for dead carbons, less griefy
			...
			if(L.check_shields(src, 25, "[name]", MELEE_ATTACK, armour_penetration_flat))
				return FALSE
			L.adjustBruteLoss(25)
			L.Weaken(2 SECONDS)
			playsound(src, 'modular_ss220/events/sound/creatures/terrorspiders/rip.ogg', 100, 1)
