	...
	if(ishuman(usr) && usr.stat != DEAD)
		...
		switch(H.get_frostbite_delta())
			if(FROSTBITE_VERYHIGH to INFINITY)
				status_list += "<span class='notice'>Тут очень тепло. Примерно на [H.get_frostbite_delta()] единиц. Я быстро тут согреюсь.</span>"
			if(FROSTBITE_HIGH to FROSTBITE_VERYHIGH)
				status_list += "<span class='notice'>Тут довольно тепло. Примерно на [H.get_frostbite_delta()] единиц. Тут можно согреться согреюсь.</span>"
			if(FROSTBITE_LOW to FROSTBITE_HIGH)
				status_list += "<span class='notice'>Тут нормальная температура. Примерно [H.get_frostbite_delta()] единиц. Я не слишком рискую.</span>"
			if(FROSTBITE_VERYLOW to FROSTBITE_LOW)
				status_list += "<span class='warning'>Тут довольно холодно. Примерно на [H.get_frostbite_delta()] единиц. Лучше не задерживаться здесь надолго.</span>"
			if(-INFINITY to FROSTBITE_VERYLOW)
				status_list += "<span class='warning'>Тут очень холодно. Примерно на [H.get_frostbite_delta()] единиц. Нужно скорее уйти отсюда!</span>"
		switch(H.frostbite)
			if(FROSTBITE_CHILLED to FROSTBITE_WARM)
				status_list += "<span class='notice'>Мне очень тепло и комфортно. Примерно на [H.frostbite] единиц. Можно идти на вылазку.</span>"
			if(FROSTBITE_MILD to FROSTBITE_CHILLED)
				status_list += "<span class='notice'>Мне немного прохладно. Примерно на [H.frostbite] единиц. Лучше немного согреться.</span>"
			if(FROSTBITE_SEVERE to FROSTBITE_MILD)
				status_list += "<span class='warning'>Мне холодно. Примерно на [H.frostbite] единиц. Нужно срочно найти где согреться.</span>"
			if(FROSTBITE_HYPOTHERMIA to FROSTBITE_SEVERE)
				status_list += "<span class='warning'>Мне очень холодно. Примерно на [H.frostbite] единиц. Если я не найду где согреться, то умру!</span>"
		...
		status_list += "<span class='notice'>По ощущениям, моя одежда защищает меня от холода на [H.get_clothes_isolation()] единиц"
		status_list += "<span class='notice'>Окружающие меня объекты и погода изменяют температуру на [H.get_around_heat()] единиц"
