...
/obj/machinery/disco/proc/lights_spin()
	...
	for(var/sparkle in sparkles)
		var/obj/effect/overlay/sparkles/reveal = sparkle
		reveal.alpha = 255
	while(active)
		for(var/glowy in spotlights) // The multiples reflects custom adjustments to each colors after dozens of tests
			var/obj/item/flashlight/spotlight/glow = glowy
			if(QDELETED(glow))
