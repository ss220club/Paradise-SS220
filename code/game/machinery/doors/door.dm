			...
			if(ishuman(user))
				...
				if(istype(C))
					...
					if(A?.try_withdraw_credits(2))
						to_chat(user, "<span class='info'>ПЛАТА ЗА ОТКРЫТИЕ: 2 кредита были списаны с вашего счёта.</span>")
					else
						to_chat(user, "<span class='info'>Обнаружен бомж, плата за открытие не взымается.</span>")
				else
					to_chat(user, "<span class='info'>Обнаружен бомж, плата за открытие не взымается.</span>")
			open()
			if(isbot(user))
