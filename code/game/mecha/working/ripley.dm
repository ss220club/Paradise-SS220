...
/obj/mecha/working/ripley/examine_more(mob/user)
	...
	Вскоре после начала производства Hephaestus передала права на выпуск Ripley другим корпорациям, получая роялти по мере роста популярности экзокостюма.</i>"
	. += ""
	...
	По сей день он остается одним из самых ценных мехов, когда-либо созданных, и Hephaestus получает значительную прибыль от продаж этой устаревшей, но прочной конструкции. \
	Как и все станционные мехи, Nanotrasen приобрела лицензию на производство «Рипли» на своих предприятиях.</i>"
...
/obj/mecha/working/ripley/firefighter/examine_more(mob/user)
	..()
	. = list()
	. += "<i>Основанный на шасси «Рипли», первоначально разработанном Hephaestus Industries, «Огнеборец» представляет собой модернизацию, созданную Nanotrasen по мере расширения горнодобывающих предприятий и необходимости в прочном, огнестойком экзокостюме. \
	Уборудован термостойкой защитой, «Огнеборец» стал популярным экзокостюмом среди горнодобывающих компаний, работающих в опасных условиях.</i>"
	. += ""
	. += "<i>С момента экспансии Nanotrasen на Эпсилон Эридани и их горнодобывающих операций в Лавалэнде, «Огнеборец» стал более популярным среди опытных шахтеров, ищущих более безопасный и надёжный способ добычи полезных ископаемых даже в самых жарких условиях. \
	Кроме того, он нашел некоторое применение среди атмосферных техников, которые ценят его способностью контролировать даже самые сильные плазменные пожары, одновременно защищая своего пилота.</i>"

/obj/mecha/working/ripley/deathripley
	desc = "ОХ ЧЁРТ, ЭТО ОТРЯД СМЕРТИ, МЫ ВСЕ УМРЕМ!"
	name = "DEATH-RIPLEY"
...
/obj/mecha/working/ripley/deathripley/examine_more(mob/user)
	..()
	. = list()
	. += "<i>Функциональный, хорошо сделанный коллекционный мез для популярного головид-шоу «Отряд смерти», финансируемого Nanotrasen. \
...
/obj/mecha/working/ripley/emag_act(mob/user)
	if(!emagged)
		emagged = TRUE
		desc += "</br><span class='danger'>Оборудование меха искрится!</span>"
	return ..()
