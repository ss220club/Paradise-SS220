...
/obj/mecha/nkarrdem/examine_more(mob/user)
	..()
	. = list()
	. += "<i>Nkarrdem — это мех старого поколения для ядерной, химической, биологической защиты, разработанный совместно компаниями Interdyne и Shellguard для быстрой деконтаминации в период частого применения биологического оружия. Однако он пострадал от ряда мер по снижению затрат, что подвергает оператора опасным условиям. Лишние единицы затем продаются для уборки..</i>"
	. += ""
...
/obj/mecha/nkarrdem/moved_inside(mob/living/carbon/human/H)
	...
	if(. && ishuman(H))
		if(istype(H.glasses, /obj/item/clothing/glasses/hud))
			occupant_message("<span class='warning'>[H.glasses.chassis.declent_ru(NOMINATIVE)] мешают вам использовать встроенный уборочный HUD [chassis.declent_ru(NOMINATIVE)].</span>")
		else
