...
/obj/item/melee/blood_magic/armor/afterattack(atom/target, mob/living/carbon/user, proximity)
	...
	if(iscarbon(target) && proximity)
		...
		C.visible_message("<span class='warning'>Чужеродная [armour ? "броня" : "снаряжение"] внезапно появляется на [C]!</span>")
		..()
...
/obj/item/melee/blood_magic/empower/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(proximity_flag)
		...
		// Veil Shifter
			...
			if(S.uses < 4)
				uses--
				to_chat(user, "<span class='warning'>Вы усиливаете [target] с помощью крови, перезаряжая его способность к сдвигу!</span>")
				playsound(user, 'sound/magic/cult_spell.ogg', 25, TRUE, SOUND_RANGE_SET(7))
				S.uses = 4
...
/obj/item/melee/blood_magic/manipulator/examine(mob/user)
	...
	. += "Кровавая сфера и кровавое усиление стоят [BLOOD_ORB_COST] и [BLOOD_RECHARGE_COST] зарядов соответственно."
...
/obj/item/melee/blood_magic/manipulator/proc/heal_human_damage(mob/living/carbon/human/user, mob/living/carbon/human/H)
	...
	if(overall_damage == 0)
		to_chat(user, "<span class='warning'>[H] не нужно лечение!</span>")
		return

	var/ratio = uses / overall_damage
	if(H == user)
		to_chat(user, "<span class='warning'>Кровавое исцеление менее эффективно при использовании на себе!</span>")
	...
	if(ratio > 1)
		...
		H.visible_message("<span class='warning'>[H] is fully healed by [H == user ? "[H.p_their()] " : "Магии Крови [H]"] blood magic!</span>",
...
/obj/item/melee/blood_magic/manipulator/proc/heal_cultist(mob/living/carbon/human/user, mob/living/carbon/human/H)
	if(H.stat == DEAD)
		to_chat(user, "<span class='warning'>Только руна возрождения может вернуть к жизни мёртвых!</span>")
		return
	...
	if(!uses)
		to_chat(user, "<span class='danger'>Вы используете последние заряды для лечения [H == user ? "себя" : "[H]"], и заклинание расстворяется!</span>")
	else
		to_chat(user, "<span class='cultitalic'>Вы используете [charge_loss] заряд(-ов), и у вас осталось [uses] зарядов.</span>")
...
/obj/item/melee/blood_magic/manipulator/proc/heal_construct(mob/living/carbon/human/user, mob/living/simple_animal/M)
	...
	if(!missing)
		to_chat(user, "<span class='warning'>[M] не нужно лечение!</span>")
		return
	if(uses > missing)
		M.adjustHealth(-missing)
		M.visible_message("<span class='warning'>[M] полностью исцелён с помощью Магии Крови [user]!</span>",
			"<span class='cultitalic'>You are fully healed by [user]'s blood magic!</span>")
		uses -= missing
	else
		M.adjustHealth(-uses)
		M.visible_message("<span class='warning'>[M] частично был вылечен с помощью Магии Крови [user]!</span>",
			"<span class='cultitalic'>Вы были частично вылечены [user].</span>")
		uses = 0
...
/obj/item/melee/blood_magic/manipulator/proc/steal_blood(mob/living/carbon/human/user, mob/living/carbon/human/H)
	if(H.stat == DEAD)
		to_chat(user, "<span class='warning'>[H.p_their(TRUE)] перестал истекать кровью, вам придётся найти другой способ её выкачивания.</span>")
		return
...
/obj/item/melee/blood_magic/manipulator/proc/blood_draw(atom/target, mob/living/carbon/human/user)
	...
	if(temp)
		...
		temp = round(temp)
		to_chat(user, "<span class='cultitalic'>Ваш кровавый обряд получил [temp] зарядов от источников крови вокруг вас!</span>")
		uses += max(1, temp)
	...
	switch(choice)
		if("Blood Orb (50)")
			if(uses < BLOOD_ORB_COST)
				to_chat(user, "<span class='warning'>Вам требуется [BLOOD_ORB_COST] зарядов для исполнения данного ритуала.</span>")
			else
				...
				if(ammount < 50) // No 1 blood orbs, 50 or more.
					to_chat(user, "<span class='warning'>Вам нужно отдать как минимум 50 единиц крови.</span>")
					return
				if(ammount > uses) // No free blood either
					to_chat(user, "<span class='warning'>У вас нет столько крови!</span>")
					return
				uses -= ammount
				...
				if(user.put_in_hands(rite))
					...
					"<span class='cult'>A [rite.name] материализуется у ваших ног.</span>")

		if("Blood Recharge (75)")
			if(uses < BLOOD_RECHARGE_COST)
				to_chat(user, "<span class='cultitalic'>Вам требуется [BLOOD_RECHARGE_COST] зарядов для исполнения ритуала.</span>")
			else
				var/obj/rite = new /obj/item/melee/blood_magic/empower()
				uses -= BLOOD_RECHARGE_COST
				qdel(src)
				if(user.put_in_hands(rite))
					to_chat(user, "<span class='cult'>Ваша рука светится мощью!</span>")
				else
					to_chat(user, "<span class='warning'>Вам нужна свободная рука для использования этого обряда!</span>")
					uses += BLOOD_RECHARGE_COST // Refund the charges
					qdel(rite)

		if("Blood Spear (150)")
			if(uses < BLOOD_SPEAR_COST)
				to_chat(user, "<span class='warning'Вам требуется [BLOOD_SPEAR_COST] charges to perform this rite.</span>")
			else
		...
		if("Blood Bolt Barrage (300)")
			if(uses < BLOOD_BARRAGE_COST)
				to_chat(user, "<span class='cultitalic'>Вам требуется [BLOOD_BARRAGE_COST] зарядов для использования данного обряда.</span>")
			else
				...
				user.drop_item()
				if(user.put_in_hands(rite))
					to_chat(user, "<span class='cult'>Обе ваших руки светятся мощью!</span>")
				else
					to_chat(user, "<span class='warning'>Вам нужна свободная рука для использования обряда!</span>")
