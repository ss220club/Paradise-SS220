...
/obj/item/clothing/suit/hooded/cultrobes/cult_shield/hit_reaction(mob/living/carbon/human/owner, atom/movable/hitby, attack_text = "the attack", final_block_chance = 0, damage = 0, attack_type = MELEE_ATTACK)
	if(current_charges && !owner.holy_check())
		owner.visible_message("<span class='danger'>[attack_text] был отражён с всплеском кровавокрасных вспышек!</span>")
		current_charges--
		...
		if(!current_charges)
			owner.visible_message("<span class='danger'>Руничесчкий щит вокруг [owner] внезапно исчез!</span>")
			shield_state = "broken"
