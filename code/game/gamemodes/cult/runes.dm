...
/obj/effect/rune/convert/proc/do_convert(mob/living/convertee, list/invokers)
	if(length(invokers) < 2)
		...
		to_chat(convertee, "<span class='cultitalic'><b>Кровь и голова пульсирует, а мир становится красным. Внезапно вы осознаёте ужасную, чудовищную правду. Завеса реальности была сорвана \
		...
		if(ishuman(convertee))
			...
			if(brutedamage || burndamage) // If the convertee is injured
				...
				if(ismachineperson(H))
					...
					"<span class='cultitalic'>Ваши раны были исцелены. Теперь пускайте кровь других.</span>")
...
/obj/effect/rune/teleport/proc/handle_portal(portal_type, turf/origin)
	...
	if(portal_type == "space")
		...
		if(is_station_level(origin.z))
			desc += "<u><span class='warning'>Кажется, бездна хочет потянуть вас в сторону [dir2text(get_dir(T, origin))], недалеко от станции!</span></u>"
		// Space base on another Z-level
		else
			desc += "<u><span class='warning'>Кажется, бездна хочет понятуть вас в сторону [dir2text(get_dir(T, origin))], в направлении [origin.z] сектора космоса!</span></u>"
...
/obj/effect/rune/raise_dead
	...
	cultist_desc = "требуется мёртвый, живой, неразумный или неактивный культист на руне. За каждую третью жертву тёмному божеству, одно тело может быть исцелено, а разум в теле - пробуждён. Лечение живых культистов требует двух аколитов вокруг руны"
...
/obj/effect/rune/raise_dead/proc/revive_alive(mob/living/target)
	target.visible_message("<span class='warning'>Тёмная магия начинает окружать [target], восстанавливая их тело.</span>")
	if(!do_after(target, 10 SECONDS, FALSE, target, allow_moving = FALSE, progress = TRUE))
		target.visible_message("<span class='warning'>Тёмная магия тихо исчезает.</span>")
		return FALSE
	target.revive()
	return TRUE
...
/obj/effect/rune/raise_dead/invoke(list/invokers)
	...
	if(diff < 0)
		to_chat(user, "<span class='cult'>Культ должен принести ещё [abs(diff)] жертв(-у) перед возрождением очередного культиста!</span>")
		fail_invoke()
		return
	...
	if(!length(potential_revive_mobs))
		to_chat(user, "<span class='cultitalic'>Отсутствуют культисты на руне!</span>")
		log_game("Raise Dead rune failed - no cultists to revive")
		fail_invoke()
		return
	if(length(potential_revive_mobs) > 1)
		mob_to_revive = tgui_input_list(user, "Выберите культиста для возрождения", "Возрождение культиста", potential_revive_mobs)
	...
	if(mob_to_revive.stat != DEAD && length(invokers) < 2)
		to_chat(user, "<span class='cultitalic'>Нужно минимум два соратника для лечения культиста!</span>")
	...
	if(!mob_to_revive.get_ghost() && (!mob_to_revive.client || mob_to_revive.client.is_afk()))
		set waitfor = FALSE
		to_chat(user, "<span class='cult'>[mob_to_revive] был возрождён, но их разум был утерен! Ищем потерянную душу для замены.</span>")
		...
		if(length(candidates) && !QDELETED(mob_to_revive))
			...
			to_chat(mob_to_revive, "<span class='biggerdanger'>Ваше физическое тело было взято другой душой ввиду вашей неактивности! Напишите adminhelp(F1) для восстановления контроля над телом.</span>")
	...
	if(!revived_from_dead)
		mob_to_revive.visible_message("<span class='warning'>[mob_to_revive] делает глубокий вдох, с красным светом из глаз.</span>", \
								"<span class='cultlarge'>Все ваши ранения исчезли!</span>")
		rune_in_use = FALSE
		return
	...
	mob_to_revive.visible_message("<span class='warning'>[mob_to_revive] делает глубокий вдох с красным светом из глаз.</span>", \
								"<span class='cultlarge'>Вы пробудились из бездны. Вы живы!</span>")
	rune_in_use = FALSE
...
/obj/effect/rune/raise_dead/proc/validness_checks(mob/living/target_mob, mob/living/user)
	...
	var/turf/T = get_turf(src)
	if(target_mob.loc != T)
		to_chat(user, "<span class='cultitalic'>Культист, которого хотели воскресить, был перемещён!</span>")
...
/obj/effect/rune/summon/invoke(list/invokers)
	...
	if(cultist_to_summon.pulledby || cultist_to_summon.buckled)
		...
		to_chat(cultist_to_summon, "<span class='cult'>Вы ощущаете тянущее чувство, но вы не можете сдвинуться!")
		fail_invoke()
		log_game("Summon Cultist rune failed - target restrained")
		return
	if(!iscultist(cultist_to_summon))
		to_chat(user, "<span class='cultitalic'>[cultist_to_summon] более не культист [SSticker.cultdat.entity_title3]!</span>")
		fail_invoke()
	...
	cultist_to_summon.visible_message("<span class='warning'>[cultist_to_summon] внезапно исчезает со вспышкой красного света!</span>", \
									"<span class='cultitalic'><b>Сильное головокружение одолевает вас в то время как вас проносят сквозь воздух!</b></span>")
	..()
	INVOKE_ASYNC(src, PROC_REF(teleport_effect), cultist_to_summon, get_turf(cultist_to_summon), src)
	visible_message("<span class='warning'>[src] начинает пузыриться и восстаёт в форме [cultist_to_summon]!</span>")
...
/obj/effect/rune/blood_boil/invoke(list/invokers)
	...
	// Six seconds buildup
	visible_message("<span class='warning'>Над [src] начинает формироваться туман!</span>")
	animate(src, color = "#FC9A6D", time = 6 SECONDS)
	set_light(6, 1, color)
	sleep(6 SECONDS)
	visible_message("<span class='boldwarning'>[src] становится ярким, пламеным оранжевым!</span>")
	if(!burn_check())
		return

	for(var/I in targets)
		to_chat(I, "<span class='userdanger'>Ваша кровь кипит в венах!</span>")
	do_area_burn(T, 1)
...
/obj/effect/rune/manifest/proc/summon_ghosts(mob/living/user, turf/T)
	...
	if(!length(ghosts_on_rune))
		to_chat(user, "<span class='cultitalic'>Нет духов возле руны!</span>")
		fail_invoke()
	...
	user.visible_message("<span class='warning'>Облако красного тумана формируется над [src], из него выходит...[new_human.gender == FEMALE ? "женщина" : "мужчина"].</span>",
						"<span class='cultitalic'>Ваша кровь начинает перетекать в [src]. Вы должны оставаться на месте и в сознании для поддержания формы призванных призраков. Медленно, но уверенно вы будете получать урон...</span>")
...
/obj/effect/rune/manifest/proc/ghostify(mob/living/user, turf/T)
	...
	ghost.name = "Тёмный Дух [ghost.name]"
	ghost.color = "red"
	CM.Grant(ghost)
	V.Grant(ghost)
	//GM.Grant(ghost)
