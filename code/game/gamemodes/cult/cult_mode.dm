...
/datum/game_mode/cult/post_setup()
	...
	for(var/datum/mind/cult_mind in cult)
		...
		if(cult_mind.assigned_role == "Clown")
			to_chat(cult_mind.current, "<span class='cultitalic'>Тёмная сила позволила вам одолеть свою клоунскую натуру, позволяя вам использовать оружие без риска пораниться.</span>")
		...
		to_chat(cult_mind.current, "<span class='motd'>Для большей информации, проконсультируйтесь с вики: ([GLOB.configuration.url.wiki_url]/index.php/Cultist)</span>")
...
/datum/game_mode/proc/equip_cultist(mob/living/carbon/human/H, metal = TRUE)
	...
	to_chat(H, "<span class='cult'>Это поможет вам зачать культ на этой станции. Используйте это с умом, и помните - вы не одни.</span>")
...
/datum/game_mode/proc/cult_give_item(obj/item/item_path, mob/living/carbon/human/H)
	...
	if(!where)
		...
		to_chat(H, "<span class='danger'>В вашем [where] есть [item_name].</span>")
		return TRUE
...
/datum/game_mode/proc/add_cultist(datum/mind/cult_mind)
	...
	if(!(cult_mind in cult))
		...
		if(cult_mind.assigned_role == "Clown")
			to_chat(cult_mind.current, "<span class='cultitalic'>Темная сила позволила вам предолеть свою клоунскую натуру, дав возможность использовать оружие без риска пораниться.</span>")
			cult_mind.current.dna.SetSEState(GLOB.clumsyblock, FALSE)
			singlemutcheck(cult_mind.current, GLOB.clumsyblock, MUTCHK_FORCED)
			var/datum/action/innate/toggle_clumsy/A = new
			A.Grant(cult_mind.current)
		...
		cult_mind.current.create_log(CONVERSION_LOG, "Обращён в культ")
		...
		check_cult_size()
		cult_objs.study(cult_mind.current)
		to_chat(cult_mind.current, "<span class='motd'>Для больше информации, проконсультруйтесь с вики: ([GLOB.configuration.url.wiki_url]/index.php/Cultist)</span>")
		...
		if(remove_gear) // No flagellants robe for non-cultists
			for(var/I in H.contents)
				if(is_type_in_list(I, CULT_CLOTHING))
					H.unEquip(I)
		if(cult_mind.assigned_role == "Clown")
			to_chat(H, "<span class='sans'>Вы свободны от тёмных сил, сдерживающих вашу клоунскую натуру. Вы снова неуклюжи! Хонк!</span>")
			H.dna.SetSEState(GLOB.clumsyblock, TRUE)
			singlemutcheck(H, GLOB.clumsyblock, MUTCHK_FORCED)
			for(var/datum/action/innate/toggle_clumsy/A in H.actions)
				A.Remove(H)
		cult_mind.current.create_log(CONVERSION_LOG, "Деконвертирован из культа")
	check_cult_size()
	if(show_message)
		...
		"<span class='userdanger'>Незнакомый яркий свет проходит через ваше сознание, очищая скверну [SSticker.cultdat ? SSticker.cultdat.entity_title1 : "Нар'Си"] и вместе с этим вашу память как её слуга.</span>")
...
/datum/game_mode/proc/cult_rise()
	...
	for(var/datum/mind/M in cult)
		...
		to_chat(M.current, "<span class='cultlarge'>С ростом культа Завеса слабеет, а глаза начинают светиться...</span>")
...
/datum/game_mode/proc/cult_ascend()
	...
	for(var/datum/mind/M in cult)
		...
		to_chat(M.current, "<span class='cultlarge'>Ваш культ вознёсся и кровавая жатва близка - вы больше не можете скрывать свою истинную сущность!</span>")
...
/datum/game_mode/proc/cult_fall()
	...
	for(var/datum/mind/M in cult)
		...
		to_chat(M.current, "<span class='cultlarge'>Завеса исцеляется, а ваша сила слабеет...</span>")
...
/datum/game_mode/cult/declare_completion()
	if(cult_objs.cult_status == NARSIE_HAS_RISEN)
		...
		to_chat(world, "<span class='danger'> <FONT size = 3>Победа культа! Им удалось призвать [SSticker.cultdat.entity_name]!</FONT></span>")
