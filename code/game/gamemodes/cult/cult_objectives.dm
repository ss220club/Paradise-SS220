...
/datum/cult_objectives/proc/study(mob/living/M, display_members = FALSE) //Called by cultists/cult constructs checking their objectives
	...
	switch(cult_status)
		if(NARSIE_IS_ASLEEP)
			to_chat(M, "<span class='cult'>[SSticker.cultdat ? SSticker.cultdat.entity_name : "The Dark One"] в спячке.</span>")
		if(NARSIE_DEMANDS_SACRIFICE)
			if(!length(presummon_objs))
				to_chat(M, "<span class='danger'>Ошибка: Нет целей в списке жертвоприношения. Something went wrong. Oof.</span>")
			else
				...
				to_chat(M, "<span class='cult'>Завеса должна быть ослаблена перед тем как мы сможем вызвать [SSticker.cultdat ? SSticker.cultdat.entity_title1 : "The Dark One"].</span>")
				to_chat(M, "<span class='cult'>Текущая задача: [current_obj.explanation_text]</span>")
		if(NARSIE_NEEDS_SUMMONING)
			to_chat(M, "<span class='cult'>Завеса ослабла! Мы можем вызвать [SSticker.cultdat ? SSticker.cultdat.entity_title3 : "The Dark One"]!</span>")
			to_chat(M, "<span class='cult'>Текущая задача: [obj_summon.explanation_text]</span>")
		if(NARSIE_HAS_RISEN)
			to_chat(M, "<span class='cultlarge'>\"Я здесь.\"</span>")
			to_chat(M, "<span class='cult'>Текущая задача:</span> <span class='cultlarge'>\"Feed me.\"</span>")
		if(NARSIE_HAS_FALLEN)
			to_chat(M, "<span class='cultlarge'>[SSticker.cultdat ? SSticker.cultdat.entity_name : "The Dark One"] испарилась!</span>")
			to_chat(M, "<span class='cult'>Текущая задача: Убивайте неверующих!</span>")
		else
	...
	if(display_members)
		...
		var/overview = "<span class='cultitalic'><br><b>Количество членов культа: [total_cult]"
		if(!SSticker.mode.cult_ascendant)
			if(rise > 0)
				overview += " | Конвертаций до Подъёма: [rise]"
			else if(ascend > 0)
				overview += " | Конвертаций до Вознесения: [ascend]"
		to_chat(M, "[overview]</b></span>")

		if(cult[2]) // If there are any constructs, separate them out
			to_chat(M, "<span class='cultitalic'><b>Культистов:</b> [cult[1]]")
			to_chat(M, "<span class='cultitalic'><b>Конструктов:</b> [cult[2]]")
	...
	if(current_obj.find_target())
		for(var/datum/mind/cult_mind in SSticker.mode.cult)
			if(cult_mind && cult_mind.current)
				to_chat(cult_mind.current, "<span class='danger'>[SSticker.cultdat.entity_name]</span> бормочет, <span class='cultlarge'>Цель культа вне досягаемости. Взамен, принесите в жертву [current_obj.target]...<span>")
		return TRUE
	return FALSE
...
/datum/cult_objectives/proc/succesful_sacrifice()
	...
	if(sacrifices_done >= sacrifices_required)
		...
		if(obj_sac.find_target())
			...
			for(var/datum/mind/cult_mind in SSticker.mode.cult)
				if(cult_mind && cult_mind.current)
					to_chat(cult_mind.current, "<span class='cult'>Вы и ваши собратья добились успеха, но ещё многое нужно сделать перед тем, как мы сможем вызвать [SSticker.cultdat ? SSticker.cultdat.entity_title1 : "The Dark One"]!</span>")
					to_chat(cult_mind.current, "<span class='cult'>Текущая задача: [obj_sac.explanation_text]</span>")
		else
			ready_to_summon()
...
/datum/cult_objectives/proc/ready_to_summon()
	...
	for(var/datum/mind/cult_mind in SSticker.mode.cult)
		if(cult_mind && cult_mind.current)
			to_chat(cult_mind.current, "<span class='cult'>Вы и ваши собратья подготовили станцию для решающего ритуала!</span>")
			to_chat(cult_mind.current, "<span class='cult'>Текущая задача: [obj_summon.explanation_text]</span>")
...
/datum/objective/servecult //Given to cultists on conversion/roundstart
	explanation_text = "Помогите вашим собратьям разорвать завесу! (Используйте Изучение завесы для просмотра прогресса.)"
	completed = TRUE
	needs_target = FALSE

/datum/objective/sacrifice
	var/sacced = FALSE
	explanation_text = "Принесите в жертву члена экипажа для подготовки призыва."
...
/datum/objective/sacrifice/find_target()
	...
	if(length(target_candidates))
		...
		explanation_text = "Принесите в жертву [target], [target.assigned_role] при помощи руны Предложения [target.p_their()] с телом или мозгом на ней и тремя культистами вокруг неё."
		return TRUE
...
/datum/objective/eldergod/proc/find_summon_locations(reroll = FALSE)
	...
	while(length(summon_spots) < SUMMON_POSSIBILITIES && sanity < 100)
		...
		if(valid_spot)
			summon_spots += summon
		sanity++
	explanation_text = "Призовите [SSticker.cultdat ? SSticker.cultdat.entity_name : "вашего бога"] с помощью руны 'Разрыв Завесы'  c девятью культистами, конструктами или призванными призраками на ней.\
	\nПризыв может быть выполнен только в [english_list(summon_spots)] - где Завеса достаточно слаба для начала ритуала."
