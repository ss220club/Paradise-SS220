...
/datum/cult_objectives/proc/study(mob/living/M, display_members = FALSE) //Called by cultists/cult constructs checking their objectives
	...
	switch(cult_status)
		...
		if(NARSIE_DEMANDS_SACRIFICE)
			if(!length(presummon_objs))
				...
				to_chat(M, "<span class='cult'>Завеса должна быть ослаблена перед призывом [SSticker.cultdat ? SSticker.cultdat.entity_title1 : "The Dark One"].</span>")
				to_chat(M, "<span class='cult'>Текущая цель: [current_obj.explanation_text]</span>")
		...
		if(NARSIE_HAS_FALLEN)
			to_chat(M, "<span class='cultlarge'>[SSticker.cultdat ? SSticker.cultdat.entity_name : "The Dark One"] испарился!</span>")
			to_chat(M, "<span class='cult'>Текущая цель: Убивайте неверных!</span>")
		else
	...
	if(display_members)
		...
		var/overview = "<span class='cultitalic'><br><b>Текущая численность культа: [total_cult]"
		if(!SSticker.mode.cult_ascendant)
			if(rise > 0)
				overview += " | Конвертаций до подъёма: [rise]"
			else if(ascend > 0)
				overview += " | Конвертаций до вознесений: [ascend]"
		to_chat(M, "[overview]</b></span>")

		if(cult[2]) // If there are any constructs, separate them out
			to_chat(M, "<span class='cultitalic'><b>Культисты:</b> [cult[1]]")
			to_chat(M, "<span class='cultitalic'><b>Конструкты:</b> [cult[2]]")
	...
	if(current_obj.find_target())
		for(var/datum/mind/cult_mind in SSticker.mode.cult)
			if(cult_mind && cult_mind.current)
				to_chat(cult_mind.current, "<span class='danger'>[SSticker.cultdat.entity_name]</span> бормочет, <span class='cultlarge'>Наша цель вне зоны досягаемости. Взамен принесите в жертву [current_obj.target]...</span>")
		return TRUE
	return FALSE
...
/datum/cult_objectives/proc/succesful_sacrifice()
	...
	if(sacrifices_done >= sacrifices_required)
		...
		if(obj_sac.find_target())
			...
			for(var/datum/mind/cult_mind in SSticker.mode.cult)
				if(cult_mind && cult_mind.current)
					to_chat(cult_mind.current, "<span class='cult'>Вы и другие аколиты добились успехов, но требуется ещё многое перед призывом [SSticker.cultdat ? SSticker.cultdat.entity_title1 : "The Dark One"]!</span>")
					to_chat(cult_mind.current, "<span class='cult'>Текущая цель: [obj_sac.explanation_text]</span>")
		else
			ready_to_summon()
...
/datum/cult_objectives/proc/ready_to_summon()
	...
	for(var/datum/mind/cult_mind in SSticker.mode.cult)
		if(cult_mind && cult_mind.current)
			to_chat(cult_mind.current, "<span class='cult'>Вам и другим аколитам удалось подготовить станцию к решающему ритуалу!</span>")
			to_chat(cult_mind.current, "<span class='cult'>Текущая цель: [obj_summon.explanation_text]</span>")
...
/datum/objective/servecult //Given to cultists on conversion/roundstart
	explanation_text = "Помогите вашим соратникам в разрыве Завесы! (Используйте 'Изучение Завесы' для просмотра прогресса.)"
	completed = TRUE
	needs_target = FALSE

/datum/objective/sacrifice
	var/sacced = FALSE
	explanation_text = "Принесите в жертву члена экипажа для подготовки к призыву."
...
/datum/objective/sacrifice/find_target()
	...
	if(length(target_candidates))
		...
		explanation_text = "Принесите в жертву [target], [target.assigned_role] при помощи руны Подношения с их телом или мозгом на ней и тремя аколитами вокруг руны."
		return TRUE
	message_admins("Жертвы Культа: Невозможно найти подходящие цели. Призыв Нар'Си разблокирован!")
	return FALSE
...
/datum/objective/eldergod/proc/find_summon_locations(reroll = FALSE)
	...
	while(length(summon_spots) < SUMMON_POSSIBILITIES && sanity < 100)
		...
		if(valid_spot)
			summon_spots += summon
		sanity++
	explanation_text = "Призовите [SSticker.cultdat ? SSticker.cultdat.entity_name : "вашего Бога"] при помощи руны 'Разрыв Завесы' с девятью культистами, конструктами или призванными духами на ней.\
	\nПризыв может быть выполнен только в [english_list(summon_spots)] - там Завеса достаточно слаба для начала ритуала."
