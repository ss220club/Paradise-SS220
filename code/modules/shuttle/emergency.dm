...
*/
		if(SHUTTLE_DOCKED)
			...
			if(time_left <= 50 && !sound_played) //4 seconds left - should sync up with the launch
				sound_played = 1
				var/hyperspace_sound = sound('modular_ss220/aesthetics_sounds/sound/hyperspace_begin_new.ogg') //SS220 EDIT
			...
			if(time_left <= 0 && !length(SSshuttle.hostile_environments))
				...
				var/hyperspace_progress_sound = sound('modular_ss220/aesthetics_sounds/sound/hyperspace_progress_new.ogg') //SS220 EDIT
				for(var/area/shuttle/escape/E in world)
		...
		if(SHUTTLE_ESCAPE)
			if(time_left <= 0)
				...
				var/hyperspace_end_sound = sound('modular_ss220/aesthetics_sounds/sound/hyperspace_end_new.ogg')
				for(var/area/shuttle/escape/E in world)
