...
/datum/reagent/consumable/drink/berry_banned/on_mob_life(mob/living/M)
	...
	if(prob(10))
		...
		to_chat(M, SPAN_NOTICE("Вы чувствуете себя немного моложе!"))
	return ..() | update_flags
...
/datum/reagent/consumable/drink/berry_banned2/on_mob_life(mob/living/M)
	...
	if(prob(10))
		to_chat(M, SPAN_NOTICE("Вы чувствуете себя немного моложе!"))		//meta this!
	return ..() | update_flags
...
/datum/reagent/consumable/drink/berry_banned2/on_mob_death(mob/living/M)
	...
	to_chat(M, chat_box_red(SPAN_ADMINHELP("ЛС от <b>Администратор</b>: БВОНЬК!")), MESSAGE_TYPE_ADMINPM, confidential = TRUE)
	..()
