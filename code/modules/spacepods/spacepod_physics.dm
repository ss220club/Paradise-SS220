/obj/spacepod/process(time)
	...
	if(velocity_mag || angular_velocity)
		...
		for(var/turf/T in locs)
			...
			drag += 0.001
			var/floating = FALSE
			if(has_gravity() && !brakes && velocity_mag > 0.1 && cell && cell.use((is_mining_level(z) ? 3 : 15) * time))
				floating = TRUE // want to fly this shit on the station? Have fun draining your battery.
			if((!floating && has_gravity()) || brakes) // brakes are a kind of magboots okay?
				drag += is_mining_level(z) ? 0.1 : 0.5 // some serious drag. Damn. Except lavaland, it has less gravity or something
				if(velocity_mag > 5 && prob(velocity_mag * 4) && istype(T, /turf/simulated/floor))
...
/obj/spacepod/Bumped(atom/movable/A) // RMNZ: Thrown items or pushable objects slightly move pods though
	if(ismob(A)) // I don't like it when little silly humans can move these pods
		return
	if(A.dir & NORTH)
