...
/proc/set_security_level(level)
	...
	if(level >= SEC_LEVEL_GREEN && level <= SEC_LEVEL_DELTA && level != GLOB.security_level)
		...
		switch(level)
			if(SEC_LEVEL_GREEN)
				GLOB.security_announcement.Announce("Все угрозы для станции устранены. Все оружие должно быть в кобуре, и законы о конфиденциальности вновь полностью соблюдаются.","ВНИМАНИЕ! Уровень угрозы понижен до ЗЕЛЁНОГО.", new_sound2 = 'sound/AI/green.ogg') //SS220 EDIT
			...
			if(SEC_LEVEL_BLUE)
				if(GLOB.security_level < SEC_LEVEL_BLUE)
					GLOB.security_announcement.Announce("Станция получила надежные данные о возможной враждебной активности на борту. Служба Безопасности может держать оружие на виду.","ВНИМАНИЕ! Уровень угрозы повышен до СИНЕГО", //SS220 EDIT
					new_sound = 'sound/misc/notice1.ogg',
					new_sound2 = 'sound/AI/blue.ogg')
				else
					GLOB.security_announcement.Announce("Непосредственная угроза миновала. Служба безопасности может больше не держать оружие в полной боевой готовности, но может по-прежнему держать его на виду. Выборочные обыски запрещены.","ВНИМАНИЕ! Уровень угрозы понижен до СИНЕГО", new_sound2 = 'sound/AI/blue.ogg') //SS220 EDIT
			...
			if(SEC_LEVEL_RED)
				if(GLOB.security_level < SEC_LEVEL_RED)
					GLOB.security_announcement.Announce("Станции грозит серьёзная опасность. Службе Безопасности рекомендуется иметь оружие в полной боевой готовности. Выборочные обыски разрешены.","ВНИМАНИЕ! КОД КРАСНЫЙ!", //SS220 EDIT
					new_sound = 'sound/misc/notice1.ogg',
					new_sound2 = 'sound/AI/red.ogg')
				else
					GLOB.security_announcement.Announce("Механизм самоуничтожения станции деактивирован, но станции по-прежнему грозит серьёзная опасность. Службе Безопасности рекомендуется иметь оружие в полной боевой готовности. Выборочные обыски разрешены.","ВНИМАНИЕ! КОД КРАСНЫЙ!", new_sound2 = 'sound/AI/red.ogg') //SS220 EDIT
					unset_stationwide_emergency_lighting()
			...
			if(SEC_LEVEL_DELTA)
				...
				GLOB.security_announcement.Announce("Механизм самоуничтожения станции задействован. Все члены экипажа обязан подчиняться всем указаниям, данными Главами отделов. Любые нарушения этих приказов наказуемы уничтожением на месте. Это не учебная тревога.","ВНИМАНИЕ! КОД ДЕЛЬТА!", //SS220 EDIT
