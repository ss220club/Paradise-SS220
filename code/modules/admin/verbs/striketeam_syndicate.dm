...
/client/proc/syndicate_strike_team()
	...
	var/input = null
	while(!input)
		input = sanitize(copytext_char(input(src, "Please specify which mission the syndicate strike team shall undertake.", "Specify Mission", ""),1,MAX_MESSAGE_LEN))	// SS220 EDIT - ORIGINAL: copytext
		if(!input)
