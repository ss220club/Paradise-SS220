...
/client/proc/gimmick_team()
	...
	var/themission = null
	while(!themission)
		themission = sanitize(copytext_char(input(src, "Please specify a briefing message for the team.", "Specify Mission", ""),1,MAX_MESSAGE_LEN))	// SS220 EDIT - ORIGINAL: copytext
		if(!themission)
