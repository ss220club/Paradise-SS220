...
/datum/emote/living/carbon/human/clap/run_emote(mob/user, params, type_override, intentional)
	...
	if(!H.bodyparts_by_name[BODY_ZONE_L_ARM] || !H.bodyparts_by_name[BODY_ZONE_R_ARM])
		if(!H.bodyparts_by_name[BODY_ZONE_L_ARM] && !H.bodyparts_by_name[BODY_ZONE_R_ARM])
			// no arms...
			to_chat(user, SPAN_WARNING("Вам нужны руки, чтобы хлопать."))
		else
...
/datum/emote/living/carbon/human/johnny/run_emote(mob/user, params, type_override, intentional)
	...
	if(!istype(H.wear_mask, /obj/item/clothing/mask/cigarette))
		to_chat(user, SPAN_WARNING("Нельзя быть настолько крутым без сигареты в зубах."))
		return TRUE

	var/obj/item/clothing/mask/cigarette/cig = H.wear_mask

	if(!cig.lit)
		to_chat(user, SPAN_WARNING("Сначала зажги [cig], модник."))
		return TRUE

	if(H.getOxyLoss() > 30)
		user.visible_message(
			SPAN_WARNING("[user] вдыхает и проглатывает сигарету!"),
			SPAN_WARNING("Вы вдыхаете и случайно проглатываете [cig]!")
		)
		if(cig.lit)
			to_chat(user, SPAN_USERDANGER("Горящая [cig] жжется по пути вниз!"))
			user.unequip(cig)
			qdel(cig)
			H.adjustFireLoss(5)
		return TRUE
...
/datum/emote/living/carbon/human/slap/run_emote(mob/user, params, type_override, intentional)
	...
	if(user.put_in_hands(smacking_hand))
		to_chat(user, SPAN_NOTICE("Вы готовы давать пощёчины."))
	else
		to_chat(user, SPAN_WARNING("В текущем положении дать пощёчину не получится."))
...
/datum/emote/living/carbon/human/snap/run_emote(mob/user, params, type_override, intentional)
	...
	if(prob(5) && !HAS_TRAIT(user, TRAIT_COOL))
		user.visible_message(SPAN_DANGER("<b>[user]</b> щелкает своими пальцами без раздумий!"))
