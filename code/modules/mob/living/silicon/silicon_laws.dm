...
/mob/living/silicon/proc/dostatelaws(method, prefix, datum/ai_laws/laws)
	if(stating_laws[prefix])
		to_chat(src, "<span class='notice'>[method]: Уже озвучиваются законы, используя данный канал связи.</span>")
		return
	...
	if(!can_state)
		to_chat(src, "<span class='danger'>[method]: Невозможно озвучить законы. Данный канал связи недоступен.</span>")
