...
/mob/living/silicon/emp_act(severity)
	...
	to_chat(src, SPAN_WARNING("Внимание: Обнаружен электромагнитный импульс."))
...
/mob/living/silicon/robot/welder_act(mob/user, obj/item/I)
	...
	. = TRUE
	if(!getBruteLoss())
		to_chat(user, SPAN_NOTICE("Нечего чинить!"))
		return
	else if(!getBruteLoss(TRUE))
		to_chat(user, SPAN_WARNING("Повреждённые компоненты не починить!"))
		return
	if(!I.use_tool(src, user, volume = I.tool_volume))
		return
	adjustBruteLoss(-30)
	add_fingerprint(user)
	user.visible_message(SPAN_ALERT("[user] латает некоторые вмятины на [src] с помощью [I]."))
...
/mob/living/silicon/proc/toggle_sensor_mode()
	to_chat(src, SPAN_NOTICE("Выберите тип сенсора."))
	...
	switch(sensor_type)
		if("Служба Безопасности")
			add_sec_hud()
			to_chat(src, SPAN_NOTICE("Включён оверлей Службы Безопасности."))
		if("Медицинский")
			add_med_hud()
			to_chat(src, SPAN_NOTICE("Включен оверлей жизненных показателей."))
		if("Диагностика")
			add_diag_hud()
			to_chat(src, SPAN_NOTICE("Включён диагностический оверлей."))
		if("Уборщик")
			add_jani_hud()
			to_chat(src, SPAN_NOTICE("Включён оверлей грязи для уборки."))
		if("Нет")
...
/mob/living/silicon/examine(mob/user)
	. = ..()
	if(silicon_hat)
		. += SPAN_NOTICE("[ru_p_they(TRUE)] [ru_p_wear()] [bicon(silicon_hat)] [silicon_hat.declent_ru(ACCUSATIVE)].")
		. += SPAN_NOTICE("Для снятия [silicon_hat.declent_ru(GENITIVE)] с [src.declent_ru(GENITIVE)] попробуй [ru_p_them()] коснуться в режиме захвата пустой рукой.")
