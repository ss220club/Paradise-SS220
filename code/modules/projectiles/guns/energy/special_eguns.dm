...
/obj/item/gun/energy/detective/proc/start_pointing(target_UID)
	...
	Announcer.autosay("Внимание: револьвер детектива отработал в режиме слежения. Отслеживание: [locateUID(tracking_target_UID)] в [get_area_name(src)].", src, "Security")
	var/obj/item/pinpointer/crew/C = locateUID(linked_pinpointer_UID)
	if(C)
		C.start_tracking()
