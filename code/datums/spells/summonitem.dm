		...
		if(!marked_item) //linking item to the spell
			...
			for(var/obj/item in hand_items)
				...
				if(istype(item, /obj/item/disk/nuclear)) //Let's not make nukies suffer with this bullshit.
					to_chat(user, SPAN_NOTICE("[item.declent_ru(NOMINATIVE)] имеет некоторую встроенную защиту от такой магии призыва."))
					break
			...
			if(!marked_item)
				if(hand_items)
					message = SPAN_CAUTION("У вас нет ничего, что можно было бы зачаровать для призыва.")
				else
					message = SPAN_NOTICE("Вы должны держать нужный предмет в руках, чтобы зачаровать его для призыва.")

		else if(marked_item && (marked_item in hand_items)) //unlinking item to the spell
			message = SPAN_NOTICE("Вы удаляете чары с [marked_item.declent_ru(GENITIVE)], чтобы использовать заклинание на чём-то другом.")
			name = "Instant Summons"
			marked_item = 		null

		else if(marked_item && !marked_item.loc) //the item was destroyed at some point
			message = SPAN_WARNING("Вы чувствуете, что ваш зачарованный предмет был уничтожен!")
			name = "Instant Summons"
			marked_item = 		null
			...
			while(!isturf(item_to_retrieve.loc) && infinite_recursion < 10) //if it's in something you get the whole thing.
				if(istype(item_to_retrieve.loc, /obj/item/organ/internal/headpocket))
					...
					if(pocket.owner)
						to_chat(pocket.owner, SPAN_WARNING("Ваш [pocket.name] внезапно становится легче. Как странно!"))
					visible_item = FALSE
					break
				if(istype(item_to_retrieve.loc, /obj/item/storage/hidden_implant)) //The implant should be left alone
					...
					for(var/mob/M in S.mobs_viewing)
						to_chat(M, SPAN_WARNING("[item_to_retrieve.declent_ru(NOMINATIVE)] внезапно исчезает!"))
					visible_item = FALSE
					break
				if(ismob(item_to_retrieve.loc)) //If its on someone, properly drop it
					...
					if(issilicon(M) || !M.transfer_item_to(item_to_retrieve, target.loc)) //Items in silicons warp the whole silicon
						M.visible_message(SPAN_WARNING("[M] внезапно исчезает!"), SPAN_DANGER("Какая-то сила внезапно отталкивает вас!"))
						M.loc.visible_message(SPAN_CAUTION("[M] внезапно появляется!"))
					...
					if(ishuman(M)) //Edge case housekeeping
						...
						for(var/X in C.bodyparts)
							...
							if(item_to_retrieve in part.embedded_objects)
								...
								to_chat(C, SPAN_WARNING("[item_to_retrieve.declent_ru(NOMINATIVE)] таинственным образом исчезает из [part.declent_ru(GENITIVE)]. Как неожиданно!"))
							...
							if(item_to_retrieve == part.hidden)
								visible_item = FALSE
								part.hidden = null
								to_chat(C, SPAN_WARNING("В [part.declent_ru(PREPOSITIONAL)] внезапно становится пусто. Как странно!"))
								break
			...
			if(!isturf(target.loc))
				to_chat(target, SPAN_CAUTION("Вы пытаетесь произнести заклинание, но у вас ничего не получается! Возможно, вы недоступны?"))
				return
			if(visible_item)
				item_to_retrieve.loc.visible_message(SPAN_WARNING("[item_to_retrieve] внезапно исчезает!"))
			...
			for(var/obj/item/disk/nuclear/N in heres_disky)
				...
				N.visible_message(SPAN_WARNING("Когда [item_to_retrieve] исчезает, [N] остается позади!"))
			...
			if(butterfingers)
				...
				item_to_retrieve.loc.visible_message(SPAN_CAUTION("[item_to_retrieve] внезапно появляется!"))
				playsound(get_turf(target),'sound/magic/summonitems_generic.ogg', 50, 1)
			else
				item_to_retrieve.loc.visible_message(SPAN_CAUTION("[item_to_retrieve] внезапно появляется в руке [target.declent_ru(GENITIVE)]!"))
				playsound(get_turf(target),'sound/magic/summonitems_generic.ogg', 50, 1)
