...
/datum/spell/inspectors_gaze/cast(list/targets, mob/living/user = usr)
	...
	if(!istype(target))
		to_chat(user, "<span class='warning'>Вы не думаете, что [target.declent_ru(NOMINATIVE)] может нарушить СРП.</span>")
		return FALSE
	to_chat(target, "<span class='warning'>Вы чувствуете, что кто-то пристально смотрит за вами...</span>")
	to_chat(user, "<span class='notice'>Вы смотрите на [target.declent_ru(ACCUSATIVE)] с намерением найти нарушения СРП.</span>")
