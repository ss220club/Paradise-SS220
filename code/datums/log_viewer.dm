...
/datum/log_viewer/proc/show_ui(mob/user)
	...
	dat += "<span>Time Search Range:</span> <a href='byond://?src=[UID()];start_time=1'>[deciseconds_to_time_stamp(time_from)]</a>" // SS220 EDIT: get rid of time2text()
	dat += " To: <a href='byond://?src=[UID()];end_time=1'>[deciseconds_to_time_stamp(time_to)]</a>" // SS220 EDIT: get rid of time2text()
	dat += "<BR>"
	...
	for(var/i in log_records)
		...
		var/time = deciseconds_to_time_stamp(L.raw_time) // SS220 EDIT: get rid of time2text()
...
/datum/log_viewer/Topic(href, href_list)
	...
	if(href_list["search"])
		...
		if(records_len > RECORD_WARN_LIMIT)
			...
			var/last_time = deciseconds_to_time_stamp(last_record.raw_time) // SS220 EDIT: get rid of time2text()
