...
/datum/uplink_item/dangerous/pistol
	name = "Пистолет Стечкина FK-69 10-мм"
	reference = "SPI"
	desc = "Небольшой, легко скрываемый пистолет, использующий патроны калибра 10-мм в магазинах на 8 патронов. Имеет \
...
/datum/uplink_item/dangerous/revolver
	...
	reference = "SR"
	desc = "Предельно простой в использовании револьвер Синдиката, стреляющий патронами калибра .357 Magnum и имеющий \
			барабан на 7 патронов. Поставляется с одним спидлоадером."
...
/datum/uplink_item/dangerous/rapid
	...
	desc = "Перчатки позволяют пользователю с невероятной скоростью наносить удары, а также толкать, хватать \
			и даже гладить своих жертв. Не увеличивают скорость атаки оружием, но отлично сочетаются с боевыми искусствами."
	reference = "RPGD"
...
/datum/uplink_item/dangerous/powerfist
	...
	desc = "Силовой кулак представляет собой металлическую перчатку со встроенным ударным поршнем, работающим от внешнего \
			 источника газа. При попадании в цель поршень выдвигается вперед и наносит серьёзный урон, отталкивая цель. \
...
/datum/uplink_item/dangerous/chainsaw
	...
	desc = "Чрезвычайно смертоносное и громкое оружие. Она не может храниться в рюкзаке и издает очень громкий шум, \
			 ...
			 Благодаря очень удобной рукояти, заведённую пилу никто не сможет вырвать у вас из рук."
	reference = "CH"
...
/datum/uplink_item/dangerous/batterer
	...
	desc = "Опасное устройство Синдиката, предназначенное для контроля толпы или организации беспорядков. Вызывает повреждения \
...
/datum/uplink_item/ammo/pistol
	...
	desc = "Дополнительный магазин на 8 патронов калибра 10-мм для пистолета Стечкина FK-69 10-мм, заряженный дешевыми патронами."
	reference = "10MM"
	item = /obj/item/ammo_box/magazine/m10mm
	cost = 2 // SS220 EDIT PRICE UP/DOWN 3 -> 2

/datum/uplink_item/ammo/pistolap
	name = "Пистолетный магазин (Бронебойные 10-мм)"
	desc = "Дополнительный магазин на 8 патронов калибра 10-мм для пистолета Стечкина FK-69 10-мм, заряженный менее эффективными \
...
/datum/uplink_item/ammo/pistolfire
	name = "Пистолетный магазин (Зажигательные 10-мм)"
	desc = "Дополнительный магазин на 8 патронов калибра 10-мм для пистолета Стечкина FK-69 10-мм, заряженный зажигательными \
...
/datum/uplink_item/ammo/pistolhp
	name = "Пистолетный магазин (Экспансивные 10-мм)"
	desc = "Дополнительный магазин на 8 патронов калибра 10-мм для пистолета Стечкина FK-69 10-мм, заряженный патронами, \
...
/datum/uplink_item/stealthy_weapons/cameraflash
	name = "Фотовспышка"
	desc = "Устройство, замаскированное под фотоаппарат, с очень яркой вспышкой. Имеет систему постепенного восполнения \
	заряда ячеек. Бесполезна против боргов и противников с защитой глаз."
	reference = "CF"
...
/datum/uplink_item/stealthy_weapons/throwingweapons
	...
	desc = "Коробка с сюрикенами и усиленными болами. Очень эффективное метательное оружие: бола запутывает ноги \
			 жертвы и быстро роняет, а сюрикен вонзается в конечность."
	reference = "STK"
...
/datum/uplink_item/stealthy_weapons/false_briefcase
	...
	оружие, что позволит стрелять прямо из портфеля. Открутите дно отверткой, вставьте оружие, и закрутите отверткой снова."
	reference = "FBBC"
...
/datum/uplink_item/stealthy_weapons/silencer
	...
	desc = "Глушитель для использования с любым малокалиберным оружием с резьбой на стволе. Заглушает выстрелы, обеспечивая \
...
/datum/uplink_item/stealthy_weapons/knuckleduster
	...
	desc = "Простое в использовании и хорошо скрываемое оружие ближнего боя, предназначенное для забивания цели до \
			 смерти жестоким способом. Это оружие разработано специально для нанесения жертве серьёзных повреждений органов."
	reference = "SKD"
...
/datum/uplink_item/explosives/plastic_explosives
	name = "Композит С-4"
	desc = "Одна из самых распространённых пластичных композитных взрывчаток. Гарантированно уничтожает объект, на \
			 котором размещена, в том числе и пол. Имеет настраиваемый таймер с минимальной задержкой до взрыва в 10 секунд. \
...
/datum/uplink_item/stealthy_tools/agent_card
	...
	desc = "Идентификационная карта, блокирующая отслеживание пользователя посредством ИИ, а также копирующая доступ \
...
/datum/uplink_item/stealthy_tools/chameleon_proj
	...
	desc = "Проецирует изображение на пользователя, маскируя его под отсканированный объект, пока он не выпустит проектор \
			 из рук или пока до маскировки не дотронутся. Пользователь в маскировке не может бежать, а снаряды пролетают сквозь него."
	reference = "CP"
...
/datum/uplink_item/stealthy_tools/emplight
	name = "ЭМИ-вспышка"
	desc = "Небольшой самозаряжающийся генератор ЭМИ ближнего действия, замаскированный под фонарик. \
...
/datum/uplink_item/device_tools/c_foam_launcher
	...
	desc = "Оружие, стреляющее пеной. Блокирует воздушные шлюзы и замедляет врагов при попадании."
	reference = "CFOAM"
...
/datum/uplink_item/device_tools/powersink
	...
	desc = "При подключении к электросети и последующем включении это огромное устройство создаёт чрезмерную нагрузку \
			 на сеть, что приводит к отключению электроэнергии по всей станции. Устройство невозможно переносить \
			  из-за его больших размеров, но вы получите небольшой маячок, который при активации телепортирует \
			   поглотитель к вашему местоположению."
	reference = "PS"
...
/datum/uplink_item/device_tools/singularity_beacon
	name = "Силовой Маяк"
	desc = "Будучи прикрученным к проводам с питанием от электросети и приведённым в действие, это большое \
...
/datum/uplink_item/suits/space_suit
	...
	desc = "Бронированный черно-красный скафандр. Легче аналогов НТ, складывается в рюкзаки, оснащён креплением \
...
/datum/uplink_item/suits/thermal
	...
	desc = "Модифицированный визор для модульного скафандра, позволяющий видеть организмы сквозь стены, улавливая \
			верхнюю часть инфракрасного спектра, которую объекты излучают в виде тепла и света. Также оснащён \
...
/datum/uplink_item/suits/springlock_module
	...
	desc = "Модификация механизма модульного скафандра, которая занимает всё пространство под внешней оболочкой. \
			 Этот механический экзоскелет отодвигается в сторону, когда пользователь входит, и помогает при раскрытии. \
			 В старых прототипах была критическая неисправность, которую в новых моделях удалось сделать контролируемой. \
			 Вы знаете, каково это, быть пронизанным экзоскелетом? Эта версия модуля была изменена таким образом, чтобы обеспечить \
			 практически мгновенную активацию МОД-сьюта. Он скрыт под модулем блокировки ДНК. По умолчанию он блокирует \
			 сворачивание на 10 секунд и почти моментально схлопывается при контакте с дымом. Вы можете перенастроить \
...
/datum/uplink_item/bio_chips/adrenal
	...
	desc = "Био-чип, вводимый инъекцией под кожу. Вводит смесь химических веществ, которые заживляют слабые раны, \
			избавляют организм от переутомления и оглушения, а также увеличивают скорость передвижения. \
			Доступно 3 стимуляции организма."
	reference = "AI"
