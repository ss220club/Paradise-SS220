...
/datum/controller/subsystem/tickets/mentor_tickets/Initialize()
	...
	close_messages = list("<font color='red' size='3'><b>- [ticket_name] Закрыт -</b></font>",
				"<span class='boldmessage'>Пожалуйста, старайтесь как можно более подробно описывать информацию в ментор-хелпе. Менторы не знаю всю ситуацию в которой вы находитесь, и им необходимо больше информации для предоставления вам помощи.</span>",
	...
	response_phrases = list("Известный баг" = "К сожалению, это известный баг. К счастью, он скоро будет пофикшен.",
		...
		"Как выполнить цель" = "Есть множество способов выполнить цель на антаге. Если вы собрались действовать на пролом, постарайтесь убедиться в том, что успеете войти и выйти до прибытия подкрепления. Стелс тоже может сработать, если вы будете действовать быстро и избегать лишнего внимания. Но не забывайте об РП способах! Заманить цель в ловушку гораздо интереснее разрядить в неё барабан с револьвера. Даже если вы обосрётесь, таким образом все стороны явно получат больше удовольствия. Не бойтесь обосраться!",
		"Английский тикет" = "Hello! You've reached the Russian Paradise server. Perhaps you mistaken. The English one has this name: \[Paradise Station]."
	)
