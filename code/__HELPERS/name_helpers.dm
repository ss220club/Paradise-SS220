...
/proc/generate_code_phrase()//Proc is used for phrase and response in master_controller.dm
	...
	// var/locations[] = length(SSmapping.teleportlocs) ? SSmapping.teleportlocs : drinks//if null, defaults to drinks instead. // SS220 DELETION
	var/names[] = list()
		...
		switch(pick(safety))//Chance based on the safety list.
			if(1)//1 and 2 can only be selected once each to prevent more than two specific names/places/etc.
				switch(rand(1,2))//Mainly to add more options later.
					...
					if(2)
						code_phrase += pick(GLOB.jobs)//Returns a job. // SS220 EDIT - ORIGINAL: code_phrase += pick(GLOB.joblist)
				safety -= 1
			if(2)
				switch(rand(1,2))//Places or things.
					if(1)
						code_phrase += pick(GLOB.cocktails) // SS220 EDIT - ORIGINAL: code_phrase += pick(drinks)
					if(2)
						code_phrase += pick(GLOB.locations) // SS220 EDIT - ORIGINAL: code_phrase += pick(locations)
				safety -= 2
			if(3)
				switch(rand(1,3))//Nouns, adjectives, verbs. Can be selected more than once.
					if(1)
						code_phrase += pick(GLOB.nouns) // SS220 EDIT - ORIGINAL: code_phrase += pick(nouns)
					if(2)
