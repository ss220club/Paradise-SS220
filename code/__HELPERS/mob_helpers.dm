...
/proc/set_criminal_status(mob/living/user, datum/data/record/target_records , criminal_status, comment, user_rank, list/authcard_access = list(), user_name)
	...
	target_records.fields["comments"] += "Статус - [status] был установлен от [user_name || user.name] ([user_rank]), время правки - [GLOB.current_date_string] [station_time_timestamp()]. Комментарий: [comment]"
